	...
	public void handleMenu(InventoryClickEvent e) {
		...
		if (e.getCurrentItem().getType().equals(Material.PLAYER_HEAD)) {
			...
			if (manager.getPlayerToEdit().equals(p.getName())) {
				p.closeInventory();
				api.msg(p, api.prefix + "&c&oYou cannot invite yourself... Nice try though");
				p.playSound(p.getLocation(), Sound.ENTITY_VILLAGER_HURT, 8, 1);
				return;
			}
			p.closeInventory();
			p.sendMessage(color(api.prefix + "&a&oYou have invited " + manager.getPlayerToEdit() + " to your party."));
			Manager view = BuddyUp.getMenuView(Bukkit.getPlayer(e.getCurrentItem().getItemMeta().getPersistentDataContainer()
					.get(new NamespacedKey(BuddyUp.get(), "uuid"), PersistentDataType.STRING)));
