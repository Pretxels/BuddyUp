...
public class PartyFinder extends MenuPaginated {
	...
	public String getMenuName() {
		return this.color("&5&oPlayer Finder |&m▬▬▬▬▬▬▬▬▬▬▬▬▬▬");
	}
	...
	public void handleMenu(InventoryClickEvent e) {
		...
		if (e.getCurrentItem().getType().equals(Material.PLAYER_HEAD)) {
			...
			if (manager.getPlayerToEdit().equals(p.getName())) {
				p.closeInventory();
				api.msg(p, api.prefix + "&c&oYou cannot join yourself... Nice try though");
		...
		if (players != null && !players.isEmpty()) {
			for (int i = 0; i < getMaxItemsPerPage(); i++) {
				...
				if (players.get(index) != null) {
					...
					Config data = new Config(players.get(index).getUniqueId().toString());
					
					if (api.isInParty(data, players.get(index)) && api.getPartyStatus(api.getParty(players.get(index))).equals("OPEN")) {
					if (api.getRequests(players.get(index)).contains(manager.getPlayerToFetch().getName())) {
						playerMeta.setDisplayName(this.color("(&b&lWAITING&r) [&aOPEN&r] &a&lOnline: &e&o" + players.get(index).getName()));
					} else
					if (api.getFriends(manager.getPlayerToFetch()).contains(players.get(index).getName())) {
						playerMeta.setDisplayName(this.color("(&6&lFRIENDS&r) [&aOPEN&r] &a&lOnline: &e&o" + players.get(index).getName()));
					} else if (players.get(index).getName().equals(manager.getPlayerToFetch().getName())) {
						playerMeta.setDisplayName(this.color("&a&lYOU &r[&aOPEN&r]"));
					} else {
						playerMeta.setDisplayName(this.color("[&aOPEN&r] &a&lOnline: &e&o" + players.get(index).getName()));
					}
					} else if (api.isInParty(data, players.get(index)) && api.getPartyStatus(api.getParty(players.get(index))).equals("LOCKED")) {
						if (api.getRequests(players.get(index)).contains(manager.getPlayerToFetch().getName())) {
							playerMeta.setDisplayName(this.color("(&b&lWAITING&r) [&cCLOSED&r] &a&lOnline: &e&o" + players.get(index).getName()));
						} else
						if (api.getFriends(manager.getPlayerToFetch()).contains(players.get(index).getName())) {
							playerMeta.setDisplayName(this.color("(&6&lFRIENDS&r) [&cCLOSED&r] &a&lOnline: &e&o" + players.get(index).getName()));
						} else if (players.get(index).getName().equals(manager.getPlayerToFetch().getName())) {
							playerMeta.setDisplayName(this.color("&a&lYOU &r[&cCLOSED&r]"));
						} else {
							playerMeta.setDisplayName(this.color("[&cCLOSED&r] &a&lOnline: &e&o" + players.get(index).getName()));
						}
					} else {
						if (api.getRequests(players.get(index)).contains(manager.getPlayerToFetch().getName())) {
